			...
			const selectDevice = async () => {
				try {
					const response = await axios({
						...
						data: {
							name: session.name,
							selectedGame: { game: session.game, cf: session.cf },
							money: session.amount,
							amountOfTime: session.duration,
							pcId: device.pcNumber,
						},
					});
		...
		<div id={styles.deviceEntry}>
			...
			<div id={styles.deviceOptions}>
				...
				>
					...
					>
						...
						<MenuItem value={'Red Dead Redemption 2'}>
			...
			<div id={styles.options}>
				<Fab
					...
					disabled={session.duration >= 360 || showConfirm ? true : false}
				>
					<Add />
				</Fab>
				<Fab
					...
					disabled={!session.duration || showConfirm ? true : false}
				>
					<Remove />
				</Fab>
			</div>
			{!showConfirm && (
				<div id={styles.confirmContainer}>
					<Button
						style={{ width: '80%', backgroundColor: '#1976d2', color: 'white' }}
						variant="contained"
						color="primary"
						startIcon={<Receipt />}
						onClick={() => setShowConfirm(true)}
						disabled={session.duration ? false : true}
					>
						Confirm
					</Button>
				</div>
			...
			{showConfirm && (
				<div id={styles.submitContainer}>
					<span>
						Procceed payment of &#8377;{session.amount} for{' '}
						{setSession.duration} minutes?{' '}
					</span>
